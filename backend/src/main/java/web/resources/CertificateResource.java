...
public class CertificateResource {
  ...
  public Response updateCertificate(@PathParam("id") Long id, CertificateDto certificateDto) {
    try {
      CertificateDto updatedCertificate =
          certificateService.updateCertificateDto(id, certificateDto);
  ...
  public Response removeAssignedUser(
      @PathParam("certificateId") Long certificateId, @PathParam("userId") Long userId) {
    try {
      ...
      return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
          .entity("Error removing assigned user")
          .build();
    }
  }
}
